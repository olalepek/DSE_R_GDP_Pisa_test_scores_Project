...
pca_res <- prcomp(as.matrix(x), center = TRUE, scale. = TRUE)
plot_data <- cbind(as.data.frame(pca_res$x[, 1]), labels = x[, 2])

ggplot(x,  y_kmeans, aes(x = PC1, y = PC2, colour = labels, labels= 2)) +
  geom_point()
